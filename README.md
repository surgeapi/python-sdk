...
pip install git+ssh://git@github.com/surgeapi/python-sdk.git
```
...
Simply import `AsyncSurge` instead of `Surge` and use `await` with each API call:
...
```sh
# install from the production repo
pip install 'surgeapi[aiohttp] @ git+ssh://git@github.com/surgeapi/python-sdk.git'
```
...
You can enable logging by setting the environment variable `SURGE_LOG` to `info`.

```shell
$ export SURGE_LOG=info
```
...
We are keen for your feedback; please open an [issue](https://www.github.com/surgeapi/python-sdk/issues) with questions, bugs, or suggestions.
